...
git clone https://github.com/sirpdboy/luci-app-parentcontrol.git luci/applications/luci-app-parentcontrol
git clone https://github.com/rianjskis/luci-app-filetransfer.git luci/applications/luci-app-filetransfer
git clone https://github.com/siwind/luci-app-usb_printer.git luci/applications/luci-app-usb_printer
...
#　编译的固件文件名
sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=ZTBWIFI-$(VERSION_DIST_SANITIZED)/g' include/image.mk
